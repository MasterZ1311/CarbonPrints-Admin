...
CP.registerModule((function () {
  ...
  function filterAndSortOrders(orders) {
    let result = orders.filter(o => {
      if (filterState.query) {
        ...
        const mPart = (o.parts || []).some(p => (p.name || '').toLowerCase().includes(q));
  ...
  function openRecordPaymentModal(order) {
    ...
    CP.ui.modal({
      ...
      body: bodyHtml, confirmText: 'Save Payment', cancelText: 'Cancel',
      onConfirm: async () => {
        ...
        if (isNaN(amount) || amount <= 0) { CP.ui.toast("Please enter a valid payment amount.", "danger"); return; }
        if (amount > balance) { CP.ui.toast(`Payment amount (₹ ${CP.util.fmtINR(amount)}) exceeds outstanding balance (₹ ${CP.util.fmtINR(balance)}).`, "danger"); return; }
        if (mode === 'UPI' && !txnRef) { CP.ui.toast("Transaction reference ID is required for UPI payments.", "danger"); return; }
        ...
        target.payment.status = (target.payment.paidAmount >= targetTot) ? 'settled' : (target.payment.paidAmount > 0 ? 'advance' : 'unpaid');
  ...
  function openPaymentHistoryModal(order) {
      ...
      : history.map((h, i) => `
        <tr data-history-id="${h.id}">
          ...
          <td style="white-space: nowrap;">
            ${h.proofDataUrl ? `<button type="button" class="btn btn-ghost btn-sm btn-view-proof" data-id="${h.id}">View</button><button type="button" class="btn btn-ghost btn-sm btn-del-proof" data-id="${h.id}">🗑️</button>` : '<span style="color:var(--text-muted);font-size:11px;">None</span>'}
          </td>
        </tr>
      `).join('');
    ...
    CP.ui.modal({
      ...
      body: `<div><div style="display:flex; justify-content:space-between; margin-bottom:12px; font-size:13px;"><div>Total: <strong>₹ ${CP.util.fmtINR(tot)}</strong> | Paid: <strong style="color:var(--success);">₹ ${CP.util.fmtINR(paid)}</strong></div><div>Status: ${CP.ui.badge(order.payment?.status || 'unpaid', getPaymentBadgeKind(order.payment?.status))}</div></div><div class="table-wrapper"><table class="table"><thead><tr><th>#</th><th>Date &amp; Time</th><th>Amount</th><th>Mode</th><th>Reference</th><th>Proof</th></tr></thead><tbody>${tableRows}</tbody></table></div></div>`,
      cancelText: 'Close'
    });
    ...
    if (modalContainer) {
      modalContainer.addEventListener('click', (e) => {
        ...
        if (viewBtn) {
          const entry = history.find(h => h.id === viewBtn.getAttribute('data-id'));
          if (entry && entry.proofDataUrl) {
            CP.ui.modal({
              title: `📸 Proof - ${entry.txnRef || entry.id}`,
              body: `<div style="text-align:center;"><img src="${entry.proofDataUrl}" alt="Proof" style="max-width:100%; max-height:70vh; border-radius:var(--radius);"></div>`,
              cancelText: 'Close'
            });
          }
        }
        ...
        if (delBtn) {
          const orders = CP.store.get('cp_orders', []);
          const target = orders.find(o => o.id === order.id);
          const hEntry = target?.payment?.history?.find(h => h.id === delBtn.getAttribute('data-id'));
          if (hEntry) {
  ...
  function openChangeStatusModal(order) {
    ...
    CP.ui.modal({
      ...
      body: `<div style="display:flex; flex-direction:column; gap:12px;"><div>Current: ${CP.ui.badge(curStatus.toUpperCase(), getStatusBadgeKind(curStatus))}</div><div class="field"><label for="new-order-status">Select Next Status *</label><select id="new-order-status" class="select">${options}</select></div></div>`,
      confirmText: 'Update Status', cancelText: 'Cancel',
      onConfirm: () => {
        const targetStatus = document.getElementById('new-order-status')?.value;
        if (!targetStatus) return;
        if (!CP.orders.canTransition(curStatus, targetStatus)) { CP.ui.toast(`Transition from ${curStatus} to ${targetStatus} is not allowed!`, 'warning'); return; }
        const orders = CP.store.get('cp_orders', []);
        const target = orders.find(o => o.id === order.id);
        if (target) {
  ...
  function renderView() {
    ...
    const rowsHtml = paginated.length === 0
      ? `<tr><td colspan="11" style="text-align: center; padding: 28px; color: var(--text-muted);">No orders found matching criteria.</td></tr>`
      : paginated.map(o => {
    ...
    root.innerHTML = `
      <div class="grid grid-cols-3" style="margin-bottom: 20px;">
        <div class="card" style="margin-bottom:0; padding:16px;"><div style="font-size:12px; color:var(--text-muted); font-weight:600;">TOTAL ORDERS</div><div style="font-size:24px; font-weight:800; margin-top:4px;">${summary.totalOrders}</div></div>
        <div class="card" style="margin-bottom:0; padding:16px;"><div style="font-size:12px; color:var(--text-muted); font-weight:600;">OUTSTANDING BALANCE</div><div style="font-size:24px; font-weight:800; color:var(--danger); margin-top:4px;">₹ ${CP.util.fmtINR(summary.outstandingBalance)}</div></div>
        <div class="card" style="margin-bottom:0; padding:16px;"><div style="font-size:12px; color:var(--text-muted); font-weight:600;">REVENUE THIS MONTH</div><div style="font-size:24px; font-weight:800; color:var(--success); margin-top:4px;">₹ ${CP.util.fmtINR(summary.revenueThisMonth)}</div></div>
      </div>
      <div class="card" style="padding:16px; margin-bottom:20px;">
        ...
        <div style="display:flex; gap:12px; margin-top:12px; align-items:flex-end; flex-wrap:wrap;">
          ...
          <div style="margin-left:auto; color:var(--text-muted); font-size:13px; align-self:center;">Showing ${filteredOrders.length === 0 ? 0 : startIdx + 1}-${Math.min(filteredOrders.length, startIdx + filterState.pageSize)} of ${filteredOrders.length}</div>
        </div>
      </div>
      <div class="card" style="padding:0; overflow:hidden;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; padding:12px 16px; border-top:1px solid var(--border); background:var(--surface-alt);">
          <div style="font-size:13px; color:var(--text-muted);">Page <strong>${filterState.page}</strong> of <strong>${totalPages}</strong></div>
          <div style="display:flex; gap:8px;">
            <button type="button" id="btn-prev-page" class="btn btn-secondary btn-sm" ${filterState.page <= 1 ? 'disabled' : ''}>◀ Prev</button>
            <button type="button" id="btn-next-page" class="btn btn-secondary btn-sm" ${filterState.page >= totalPages ? 'disabled' : ''}>Next ▶</button>
          </div>
        </div>
      </div>
    `;
  ...
  function bindEvents() {
    root.querySelector('#filter-query')?.addEventListener('input', CP.util.debounce((e) => {
      filterState.query = e.target.value.trim(); filterState.page = 1; renderView();
    }, 250));
    root.querySelector('#filter-status')?.addEventListener('change', (e) => { filterState.status = e.target.value; filterState.page = 1; renderView(); });
    root.querySelector('#filter-payment')?.addEventListener('change', (e) => { filterState.payment = e.target.value; filterState.page = 1; renderView(); });
    root.querySelector('#filter-category')?.addEventListener('change', (e) => { filterState.category = e.target.value; filterState.page = 1; renderView(); });
    root.querySelector('#filter-date-from')?.addEventListener('change', (e) => { filterState.dateFrom = e.target.value; filterState.page = 1; renderView(); });
    root.querySelector('#filter-date-to')?.addEventListener('change', (e) => { filterState.dateTo = e.target.value; filterState.page = 1; renderView(); });
    root.querySelector('#btn-reset-filters')?.addEventListener('click', () => {
      ...
      filterState.dateFrom = ''; filterState.dateTo = ''; filterState.page = 1; renderView();
    });
    ...
    root.querySelector('#btn-prev-page')?.addEventListener('click', () => { if (filterState.page > 1) { filterState.page--; renderView(); } });
    root.querySelector('#btn-next-page')?.addEventListener('click', () => { filterState.page++; renderView(); });

    root.addEventListener('click', (e) => {
      const row = e.target.closest('tr[data-order-id]');
      if (!row) return;
      const order = (CP.store.get('cp_orders', [])).find(o => o.id === row.getAttribute('data-order-id'));
      if (!order) return;
